...
from .activity_parser_tools import (
...
def category_lenta_parser(categories, analyze_by: str, **kwargs):
    ...
    for category_id in categories:
        ...
        if category is not None:
            category_analyzer = activity_lenta_parser(
                category=category, analyze_by=analyze_by, **kwargs
            )
            analyzed_value[categories_dict[category_id]["label"]] = sum(
                category_analyzer.values()
            )
    return analyzed_value
