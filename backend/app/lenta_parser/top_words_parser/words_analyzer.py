...
from ...config_tools.configuration import stop_terms

morph = pymorphy2.MorphAnalyzer(lang="ru")


def split_texts_to_words(texts: list):
    """
...
def get_words_in_normal_form(words: typing.Iterable):
    ...
    words = (
        ...
        for word in words
        if morph.parse(word)[0].tag.POS in {"INFN", "NOUN", "VERB"}
        and morph.parse(word)[0].normal_form not in stop_terms
    )
    return words
...
def get_only_verbs(words: typing.Iterable):
    ...
    """
    words_in_normal_form = get_words_in_normal_form(words)
    verbs = [
        word
        for word in words_in_normal_form
        if morph.parse(word)[0].tag.POS in {"INFN", "VERB"}
    ]
    return verbs
